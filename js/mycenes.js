...
function mycenes(data){
    let stationInfo = JSON.parse(data);
    let j = 0;
    $(".right").append("<h3>Station Mycènes</h3><br><table class='infoTable'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Identifiant Vélo</th><th>Disponibilité Vélo</th><th>louer Vélo</th>");
...
let mycenesBorne;
function choiceMycenes(){
    $("input[value='Choisir']").click(function(){
        mycenesBorne = $(this).attr('id');
        $.post('php/user_choose-bike.php',
        {borne_nb: mycenesBorne},
        function(data){
            userMycenes(data);
        });
    });
...
function userMycenes(data){
    let departure = "Mycenes";
    let dep_terminal = mycenesBorne;
    let usr_bike = data;
    showMapterminus(departure, dep_terminal, usr_bike);
}

function mycenes_deposit(departure, dep_terminal, usr_bike, arr_station , data){
    let count = 0;
    let stationInfo = JSON.parse(data);
    $(".right").append("<h3>Station Mycènes</h3>Distance avec la station de départ : " + distanceBetweenStation[6] + "m<br><br><table class='thisStation'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Séléctionner Borne de dépôt</th>")
