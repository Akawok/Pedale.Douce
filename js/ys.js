//AFFICHAGE FENETRE STATION YS
function ys(data){
    let stationInfo = JSON.parse(data);
    let j =0
    $(".right").append("<h3>Station Ys</h3><br><table class='infoTable'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Identifiant Vélo</th><th>Disponibilité Vélo</th><th>louer Vélo</th>");
...
function userYs(data){
    let departure = "Ys";
    let dep_terminal = ysBorne;
    let usr_bike = data;
    showMapterminus(departure, dep_terminal, usr_bike);
}

function ys_deposit(departure, dep_terminal, usr_bike, arr_station , data){
    let count = 0;
    let stationInfo = JSON.parse(data);
    $(".right").append("<h3>Station Ys</h3>Distance avec la station de départ : " + distanceBetweenStation[3] + "m<br><br><table class='thisStation'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Séléctionner Borne de dépôt</th>")
