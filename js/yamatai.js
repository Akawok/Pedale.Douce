...
function yamatai(data){
    let stationInfo = JSON.parse(data);
    let j =0
    $(".right").append("<h3>Station Yamatai</h3><br><table class='infoTable'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Identifiant Vélo</th><th>Disponibilité Vélo</th><th>louer Vélo</th>");
...
let yamataiBorne;
function choiceYamatai(){
    $("input[value='Choisir']").click(function(){
        yamataiBorne = $(this).attr('id');
        $.post('php/user_choose-bike.php',
        {borne_nb: yamataiBorne},
        function(data){
            userYamatai(data);
        });
    });
...
function userYamatai(data){
    let departure = "Yamatai";
    let dep_terminal = yamataiBorne;
    let usr_bike = data;
    showMapterminus(departure, dep_terminal, usr_bike);;
}

function yamatai_deposit(departure, dep_terminal, usr_bike, arr_station , data){
    let count = 0;
    let stationInfo = JSON.parse(data);
    $(".right").append("<h3>Station Yamatai</h3>Distance avec la station de départ : " + distanceBetweenStation[7] + "m<br><br><table class='thisStation'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Séléctionner Borne de dépôt</th>")
