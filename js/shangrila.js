...
function shangrila(data){
    let stationInfo = JSON.parse(data);
    let j =0
    $(".right").append("<h3>Station Shangri-La</h3><br><table class='infoTable'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Identifiant Vélo</th><th>Disponibilité Vélo</th><th>louer Vélo</th>");
...
let shangrilaBorne;
function choiceShangriLa(){
    $("input[value='Choisir']").click(function(){
        shangrilaBorne = $(this).attr('id');
        $.post('php/user_choose-bike.php',
        {borne_nb: shangrilaBorne},
        function(data){
            userShangrila(data);
        });
    });
...
function userShangrila(data){
    let departure = "Shangri-La";
    let dep_terminal = shangrilaBorne;
    let usr_bike = data;
    showMapterminus(departure, dep_terminal, usr_bike);
}

function shangrila_deposit(departure, dep_terminal, usr_bike, arr_station , data){
    let count = 0;
    let stationInfo = JSON.parse(data);
    $(".right").append("<h3>Station Shangri-La</h3>Distance avec la station de départ : " + distanceBetweenStation[5] + "m<br><br><table class='thisStation'><th>Borne Numéro</th><th>Disponibilité Borne</th><th>Séléctionner Borne de dépôt</th>")
